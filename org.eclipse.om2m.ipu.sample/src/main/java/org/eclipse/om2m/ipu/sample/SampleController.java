...
public class SampleController implements IpuService {
    ...
    public ResponseConfirm doExecute(RequestIndication requestIndication) {
        ...
        String deskId = info[info.length-3];
        String type = deskId.split("_")[0];
        String value = info[info.length-1];
        try {
            if(Desk.TYPE.equals(type)) {
                SampleMonitor.setDeskState(deskId, value);
    ...
    public ResponseConfirm doRetrieve(RequestIndication requestIndication) {
        ...
        try {
            ...
            value = SampleMonitor.getDeskValue(appId);
            String content = Desk.getStateRep(appId, value);
